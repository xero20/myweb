library(sf); library(viridis); library(ggplot2); library(RColorBrewer)
...
grid_1km = grid_1km %>% select(-c(PM2_5))
gc()
...
#speriod = paste0(fyear,"-",fmonth,"-",fday)
#eperiod = paste0(uyear,"-",umonth,"-",uday)

period = scan("image/file.txt", what="", sep="\n")
period_diff = as.integer(as.Date(period[2])-as.Date(period[1]))

######
data <- fread("yespm_float16.csv")
#data = data%>%select(c(geo_id,date,PM2_5))
data = data%>%select(c(geo_y_cen,geo_x_cen,date,PM2_5)) #year,month,day
...
for (i in seq(1,period_diff)) {
  #dat <- dat %>% group_by(date,geo_id) %>% summarise_all(list(mean))
  #year = data[data$year==dates[1:4]]
  #month = data[data$year==dates[6:7]]
  #day = data[data$year==dates[9:10]]
  #dat = paste0(year,month,day)
  dat = data[data$date=dates]
  dat <- grid_1km %>% right_join(dat,by=c('geo_y_cen'="geo_y_cen",'geo_x_cen'='geo_x_cen'))
  #dat <- grid_1km %>% left_join(dat,by=c('geo_y_cen'="geo_y_cen",'geo_x_cen'='geo_x_cen'))
...
#          panel.grid.major = element_blank())
  ...
  tiff(filename = paste0("image/",dates,"_web_pm25_map.tif"),
